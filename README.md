...
一至四週且具備超過 500 條合格路徑時，系統使用 expanding-window 樣本外回測。每個歷史時點只使用更早的路徑估計當時的保守／安全界線，再觀察實際到期收盤與週內最低／最高價。

下檔 Put 另外追蹤到期跌破後，未來收盤首次重新站上當時模型履約價所需的交易期數，並顯示 5／20／60 個交易日內的回復比例。接近資料尾端且觀察期不足的案例視為右截尾，不會被誤算為未在該期限內解套。回復價格未扣除已收權利金，因此不是實際淨損益兩平時間。
