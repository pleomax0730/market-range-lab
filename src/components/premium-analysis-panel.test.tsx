...
describe('PremiumAnalysisPanel', () => {
  it('shows all four auditable premium floors without an executable premium input', async () => {
    ...
    function ControlledPanel() {
      ...
      return (
        <PremiumAnalysisPanel
          candidate={candidate}
          annualCapitalReturnRatePct={annualRate}
          onAnnualCapitalReturnRatePctChange={setAnnualRate}
        />
      )
    }
    ...
    expect(screen.queryByLabelText('預估可成交淨權利金 / 股（選填）')).not.toBeInTheDocument()
    expect(screen.getByLabelText('年化資金門檻')).toHaveValue(10)

    await user.clear(screen.getByLabelText('年化資金門檻'))
    await user.type(screen.getByLabelText('年化資金門檻'), '12')
    expect(screen.getByLabelText('年化資金門檻')).toHaveValue(12)
  })
  ...
  it('warns when history contains no positive expiration payoff', () => {
    ...
    render(
      <TooltipProvider>
        <PremiumAnalysisPanel
          candidate={noLossCandidate}
          annualCapitalReturnRatePct="10"
          onAnnualCapitalReturnRatePctChange={() => undefined}
        />
      </TooltipProvider>,
    )
