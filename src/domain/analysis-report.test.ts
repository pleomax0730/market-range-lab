...
describe('Analysis Report', () => {
  it('uses the same grade-paused candidate and horizon results for view and JSON export', () => {
    ...
    expect(report.candidate?.recoveryFrontier).toBeDefined()
    expect(report.candidate?.recoveryFrontier?.settings.target).toBe('break-even')
    ...
    expect(exported.candidate.recoveryFrontier).toEqual(report.candidate?.recoveryFrontier)
    ...
    expect(exportedCsv).toContain('"candidateRecoveryFrontierDeadlinePeriods"')
    expect(exportedCsv).toContain('"candidateRecoveryFrontierIntervals"')
