...
function keysFor(input: StatisticalReportInput | undefined, analysisKey: string | undefined) {
  ...
  const candidate = input.candidate
    ? `${input.candidate.targetDate}:${input.candidate.side}:${input.candidate.price}:premium=${input.candidate.netPremiumPerShare ?? ''}:recovery=${input.candidate.recoveryThroughDate ?? ''}:${input.candidate.recoveryWindowPeriods ?? ''}:frontier=${JSON.stringify(input.candidate.recoveryFrontierSettings ?? {})}`
    : 'none'
